...
#>
    ...
    Process
    {
        if($name)
        {
            ...
            $tempvmID = ((Invoke-WebRequest -Uri $url -Method Get -Headers @{'vmware-api-session-id'=$sessionID} -ContentType 'application/json' -UseBasicParsing).Content | ConvertFrom-Json).value.vm
...
function Get-VMWareRATag {
    ...
    Process
    {
        ...
        {
            $tags | ForEach-Object {
                if ((Get-VMWareRATagByID -vCenter $vCenter -sessionID $sessionID -tagID $_).name -eq $name)
                {
                    [void] $tagList.Add((Get-VMWareRATagByID -vCenter $vCenter -sessionID $sessionID -tagID $_))
                }
            }
        }
        else{$tags | ForEach-Object {[void]$tagList.Add((Get-VMWareRATagByID -vCenter $vCenter -sessionID $sessionID -tagID $_))}}
       if ($category)
        {
            $catID = (Get-VMWareRATagCategory -vCenter $vCenter -sessionID $sessionID -category $category).ID
            $tempList = $tagList | Where-Object {$_.category_id -eq $catID}
            Remove-Variable tagList
            $tagList = $tempList
        }
        return $tagList
...
#region New-VMWareRATagCategory
function New-VMWareRATagCategory {
<#
    .Synopsis
...
#region New-VMWareRATag
function New-VMWareRATag {
<#
    .Synopsis
        Create a new Tag
    ...
    .PARAMETER Category
        Category Name the tag will be member
...
#>
    ...
    {      
        $url = "https://$vCenter/rest/com/vmware/cis/tagging/tag"
        $FoundCategory = Get-VMWareRATagCategory -vCenter $vCenter -sessionID $sessionID -category $Category
        If ($FoundCategory)
        {
          If ((Get-VMWareRATag -vCenter $vCenter -sessionID $sessionID -category $category -Name $Name).Name -eq $null)
          {
            $json = @{"create_spec" = @{"category_id"=$FoundCategory.id;"name"=$Name;"description"=$description}} | ConvertTo-Json -Depth 32   
            return (Invoke-WebRequest -Uri $url -Method Post -Body $json -Headers @{'vmware-api-session-id'=$sessionID} -ContentType 'application/json' -UseBasicParsing)
          }  
       }Else
       {
         Write-error "Category does not exists"
         return $false
       }
    }
    End
...
#region New-VMWareRATagAssignement
function New-VMWareRATagAssignement
{
<#
    .Synopsis
        Associate a tag to a vcenter object
    ...
    .PARAMETER Entity
        Array of moref you want to associate the tag
        
    .PARAMETER ObjectType
        Type of the object you want to associate the tag
        Only VirtualMachine available at the moment
...
#>
    ...
    Process
    {
        If ($Category)
        {
         $tagId = @((Get-VMWareRATag -Vcenter $vCenter -Session $session -Name $Tag -Category $Category).Id) 
        }Else
        {
         $tagId = @((Get-VMWareRATag -Vcenter $vCenter -Session $session -Name $Tag).Id) 
        }
        If ($tagid)
        {
          ## Construct url
        ...
        {
            Write-Error "Tag does not exists"
            return $false
        }
